...
def index_tags(filename):
    result = {}
    with open(filename) as f :
        for webentity in csv.DictReader(f) :
            result[webentity["ID"]] = webentity
        return result
...
if __name__=='__main__':
    # We index all the tags by WE id
    index_tags = index_tags("COP21_tags.csv")
    ...
    # hyphe core
    try:
        hyphe_core=jsonrpclib.Server(conf["hyphe-core"]["host"], version=1)
    except Exception as e:
        sys.stderr.write("%s: %s\n" % (type(e), e))
    ...
    try:
        ...
        for _ in range(conf["hyphe2solr"]["nb_process"]):
            hyphe_core_proc = Process(target=index_webentity, args=(web_entity_queue,web_entity_done,hyphe_core,coll,solr, conf['hyphe-core']['corpus_id'], index_tags))
